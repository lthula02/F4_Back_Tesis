...
while os.path.exists(f'C:\\TESISBEHRENSBRICENO\\diagrama_de_componentes_{name}v{cont}'):
    cont+=1
...
for comp in compdata:
    ...
    if len(comp['requires']) >0:
        for i, compr in enumerate(comp['requires']):
            graph.node(f'aux{comp["name"]}{i}', " ", shape="point", style='filled', fillcolor='white', width='0.1')
            graph.edge(comp['name'], f'aux{comp["name"]}{i}',  dir='forward', style='plain', arrowhead='icurve')
    
for comp in compdata:
    for i, req in enumerate(comp['requires']):

        # graph.edge(comp['name'], req, dir='both', style='plain', arrowhead='curve', arrowtail='odot')
        graph.edge(f'aux{comp["name"]}{i}', req, dir='forward', style='plain', arrowhead='none')
...
graph.node('aux',' ', shape='point', width='0.1', style='filled', fillcolor='white')
